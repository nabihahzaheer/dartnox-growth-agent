 ...
 */
const BUILD_INSTANTS: ReadonlyArray<readonly [string, string, string]> = [
  ['2026-08-17T08:00:00Z', 'a Monday — this week\'s Thursday is three days ahead', '2026-08-20'],
  ['2026-08-19T09:00:00Z', 'a Wednesday — the old rule\'s worst case, six days behind', '2026-08-20'],
  ['2026-08-20T13:30:00Z', 'a Thursday before the anchor hour — stays today', '2026-08-20'],
  ['2026-08-20T14:00:00Z', 'a Thursday exactly on the anchor hour', '2026-08-20'],
  ['2026-08-22T23:00:00Z', 'late on a Saturday — back to this week\'s Thursday', '2026-08-20'],
  ['2026-01-01T00:00:00Z', 'across a year boundary', '2026-01-01'],
  ['2026-03-01T12:00:00Z', 'a Sunday, so the Monday-anchored week is February\'s', '2026-02-26'],
];

for (const [iso, description, expectedDay] of BUILD_INSTANTS) {
  ...
  check(`${label} — is ${expectedDay}`, computeAnchorIso(buildAt).startsWith(expectedDay));
  /**
   * THREE DAYS, EITHER DIRECTION. The old assertion was "not in the future", which the anchor may
   * now be by up to three days — see the note on `computeAnchorIso`. This is the tighter bound
   * that replaced it: the old rule allowed seven days of drift as long as all of it was backwards.
   */
  check(
    `${label} — within three days of the build in either direction`,
    Math.abs(buildAt.getTime() - anchor.getTime()) <= 3 * 86_400_000 + 86_400_000,
  );
}
...
/**
 * EVERY LIVE CHILD TAKES THE SAME TIME TO WATCH.
 *
 * Four children stream side by side on the console, and their step lists come from two different
 * places: two are generated by `childSteps` in `fixtures/batch.ts` and two are written out by hand
 * in `fixtures/pipeline.ts`. When the generated pair's playback budget was raised, the hand-written
 * pair was missed — so two cards finished in about fifty seconds while the other two were a third
 * of the way through, for no reason a viewer could see. Nothing in the type system connects the two
 ...
 */
{
  ...
  const running = fixtures.runs.filter((r) => r.state === 'running');
  check('four children are mid-flight on the console', running.length === 4, `saw ${running.length}`);

  const budgets = running.map((r) => ({
    id: r.id as string,
